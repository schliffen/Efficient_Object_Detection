...
def transform_annotation_(x, orig_dim, model_dim):
    """
    ...
    if not x:
        return None
    content = x.rstrip().split('_')
    #
    category_id = int(content[0])
    # orig boxes
    boxes = np.array([float(content[1].split('-')[i]) for i in range(4)], dtype='float32')
    # center to corner
    _bboxes = np.zeros_like(boxes)
    _bboxes[0] = (boxes[0] - boxes[2]/2) * orig_dim[1]
    _bboxes[1] = (boxes[1] - boxes[3]/2) * orig_dim[0]
    _bboxes[2] = (boxes[0] + boxes[2]/2) * orig_dim[1]
    _bboxes[3] = (boxes[1] + boxes[3]/2) * orig_dim[0]

    # rescaling boxes

    return np.hstack((_bboxes, category_id))




class CustomDataset(Dataset):
    def __init__(self, root, num_classes, ann_file, cfg_file, det_transforms=None, random_data = None,
                 rgb_mean = (0,0,0), inp_dim=  416 ):
        """
        ...
        # self.random_img = random_data
        self.augmentator = ImgAugTransform( rgb_mean, random_data, inp_dim)
...
#        return super().__len__()
    ...
    def __getitem__(self, idx):
        ...
        ground_truth = transform_annotation_(example, image.shape, self.inp_dim)

        # with open( self.root + 'annotations/' + imageName.split('.')[0] + '.txt') as f:
        #     lines = f.readlines()
        ...
        _img, ground_truth =self.augmentator.augment_img_lbl( _img, np.expand_dims(ground_truth,0))
