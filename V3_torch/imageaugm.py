...
def draw_on_image(image, keypnt):
    ...
    cv2.rectangle(image, (int(keypnt[0][0]), int(keypnt[0][1])), (int(keypnt[0][2]), int(keypnt[0][3])), (255,0,0),2)
    #
...
class ImgAugTransform():
    ...
    def _resize_subtract_mean(self, image, target ):
        ...
        image -= self.rgb_mean
        image /= (image.max() + .00000001)

        target[0,:4] = np.array([target[0][i]/image.shape[(2*i+1) % 2] for i in range(4) ])

        return image, target # .transpose(2, 0, 1)

    def crop_and_paste(self, src, img, target):
        #

        num_plate = src[int(round(target[0][1])):int(round(target[0][3])),
                    int(round(target[0][0])):int(round(target[0][2]))]
        # random enlargement factor
        try:
            rratio_x = np.random.uniform(1, 2)
            rratio_y = np.random.uniform(1, 2)
            rand_x =  int(rratio_x * num_plate.shape[1])
            rand_y = int(rratio_y *  num_plate.shape[0])
            # selecting a uniform position on the target image
            num_plate = cv2.resize( num_plate, (rand_x, rand_y))
            ly, lx = num_plate.shape[:2]
            #
        ...
        img[plc_y: plc_y + ly, plc_x:plc_x+lx] = num_plate
        ...
        target[0][0], target[0][1], target[0][2], target[0][3] = plc_x, plc_y, plc_x + lx, plc_y + ly

        return img, target

    def mix_images(self, img_1, img_2):
        #
        landa = np.random.uniform(.75,.9)
    ...
    def augment_img_lbl(self, img, target):
        ...
        if  selector== 0 or selector ==1:
            ...
            image_aug, target_crp = self.crop_and_paste(img.copy(), randImg, target.copy())
            ...
            while ( kps_aug[0].x1<0 or kps_aug[0].y1<0 or kps_aug[0].x2<0 or kps_aug[0].y2<0 or \
                ...
                if cntr>tries:
                    break
            #
            target[0][0], target[0][1], target[0][2], target[0][3] = kps_aug[0].x1, kps_aug[0].y1, kps_aug[0].x2, kps_aug[0].y2
        ...
        # vis_img2 = draw_on_image(image_aug.copy(), target.copy())
