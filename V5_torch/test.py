...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='./runs/exp0_yolov5s_train/weights/last_yolov5s_train.pt', help='model.pt path(s)')
    parser.add_argument('--data', type=str, default='./data/gun_dataset.yaml', help='*.data path')
    ...
    if opt.task in ['val', 'test']:  # (default) run normally
        ...
        for weights in ['yolov5s.pt', 'yolov5m.pt', 'yolov5l.pt', 'yolov5x.pt', 'yolov3-spp.pt']:
            f = './inference/study_%s_%s.txt' % (Path(opt.data).stem, Path(weights).stem)  # filename to save to
